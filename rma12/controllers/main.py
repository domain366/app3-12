...
class odoo_website_rma(CustomerPortal):

    @http.route()
    def account(self, **kw):
        """ Add sales documents to main account page """
        response = super(odoo_website_rma, self).account(**kw)
    ...
    def portal_my_rma(self, page=1, date_begin=None, date_end=None, **kw):
        ...
        return request.render("odoo_website_rma.portal_my_rma", values)
        
        
class OdooWebsiteRMA(http.Controller):
    ...
    def website_rma(self, **post):
        ...
        stock_picking_obj = request.env['stock.picking']

        sale_order = sale_order_obj.browse(int(order_id))
        product_data = product_obj.browse(int(product_id))
        
        vals = {'partner_id': partner_id, 'order_id': order_id, 'rma_product_id': product_id, 'rma_product_uom_qty': quantity, 'reason': reason, 'rma_product_uom': product_data.uom_id.id, 'user_id': sale_order.user_id.id, 'team_id': sale_order.team_id.id, 'company_id': sale_order.company_id.id }  
        ...
        for picking in sale_order.picking_ids:
            ...
            picking_obj = request.env['stock.picking'].browse(picking)
            stock_move_ids = request.env['stock.move'].search([('picking_id','=',picking.id)])
        ...
        return request.render("odoo_website_rma.rma_thankyou")
